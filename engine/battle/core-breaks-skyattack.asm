...
; the "FIGHT" menu was selected
	xor a
	ld [wNumRunAttempts], a
	; I CAN INSERT MY "DRAW" CODE HERE. For now it's just testing if the first slot Pokemon gets the energy.
	;xor a
	ld hl, wEnergyGranted
	or [hl]
	jp nz, LoadScreenTilesFromBuffer1
	ldh [hWhoseTurn], a ; I DO NEED THIS TO AFFIRM I GET THE PP DRAWN. Well, enemy could too I suppose.
	ld a, [wPlayerMonNumber]
	ld [wWhichPokemon], a
	call DrawEnergy
	ld [wTempByteValue], a
...
.moveselected
	...
	ld c, a
	ld b, $0
	add hl, bc ; so now this points to, in vanilla, how much PP the mon has... this isn't what I need
	ld a, [hl] ; d now contains the index of the move I'm working with
	dec a ; table starts at pound, not empty_move or null_move or whatnot
	ld hl, Moves
	ld bc, MOVE_LENGTH
	call AddNTimes
	push de
	...
	ld a, BANK(Moves)
	call FarCopyData
	; now wMoveData holds the bytes I care about
	ld hl, wMoveData ; don't do ld h, d and ld l, e because de has been shifted by the FarCopyData
	ld b, $0
	ld c, $3 ; +0 is the id/animation; +1 is effect, +2 is power, +3 is types, +4 is accuracy, +5 is pp
	add hl, bc
	ld a, [hli] ; this now contains the Type 1/2 structure of Moves
	ld d, a ; d holds my type for now
	inc hl ; skip over accuracy
	ld b, [hl] ; this now contains the PP 1/2 structure of Moves
ld a, [wPlayerBattleStatus3]
bit TRANSFORMED, a
jp nz, .transformedMoveSelected ; in this case, magic happens. Both types are treated as colorless, I just need to sum the PP.
	ld a, $7 ; we're going to check if it's singletype or dualtype move
	and b
...
; print TYPE/<type> and <curPP>/<maxPP>
	...
	jr nc, .PrimaryIsColorless ; this branch needs us to evaluate what the secondary is because if it's typed like Sky Attack we need to deduct that energy before we print this one
...
.noswap
	...
	ld [hli], a
	push af ; this branch had 2 pop afs, this balances it; a and f will be overwritten momentarily
.PrimaryIsColorless
	pop af ; branch mirroring
	ld [hl], '/'
	inc hl
...
; this means we saw type1 is colorless, and type2 is probably null but could be fire due to sky attack
	...
	add $F6
	ld [hl], a
	jr .moveDisabled ; just where it normally picks up
.alternativeType2
	ld a, [wTempByteValue]
	add $F6
	sub [hl] ; this is the maxPP of second energy
